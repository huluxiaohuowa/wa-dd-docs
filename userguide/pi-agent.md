...
配置 Sciverse 后，工作台会显示“检索文献”和“查询点石”入口：点击会填入相应指令，发送后 Agent 可调用可引用的文献证据检索和点石的物质、反应、相似度与文献工具。界面会把文献标题、证据片段、页码和 `doc_id` 以科研证据卡片显示，而不是把原始 JSON 混入对话。SeqStudio 当前仅公开在线/本地工作流，没有公开 HTTP API；其 Key 不会被伪装成可调用能力。
