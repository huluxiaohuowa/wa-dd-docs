...
### 4.2 进行中
- **KRAS G12C L0 基准验证（server6）**：旧实例的任务历史未迁移，已在 server6 项目 `cryptic-pocket-kras-g12c-l0` 重建基准资产并跑通 L0 全流程（裸受体、`fep_md` 配体库、含元素变化映射的 RBFE 提交组合，完整流程已固化在执行参考 5.1 节）。APO 任务 `6273dcd5` 已完成（Adagrasib 边 ΔΔG = -7.48 ± 2.95 kcal/mol 可解释；Sotorasib 边 -99.15 ± 7.68 因映射过小不可信，已触发"低可靠性标注"开发 `7a55400`）。OPEN 任务 `484407ba` 在最终分析阶段遇 server6 网络故障，待恢复后确认/重试，随后做 ΔΔΔG 构象偏好判定。
- **OpenFold3 构象生成验证**：`AssetFile.size` worker bug 已修复（`a43b816`，2026-08-20 后镜像包含），模板扰动和配体诱导预测待在 server6 重提并筛选。
...
| **第二阶段 (Week 2)** | 用 `wa-dd-gromacs` 跑通 L0 基准 FEP + L2 物理验证。 | L0 链路已跑通（APO completed，2026-08-24）；OPEN 收尾遇主机网络故障待确认；L2 待 L1 产出 |
