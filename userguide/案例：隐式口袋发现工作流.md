...
### 4.2 进行中
- **KRAS G12C L0 基准验证（server6 重启）**：旧实例的任务历史未迁移，已在 server6 项目 `cryptic-pocket-kras-g12c-l0` 重建基准资产（含去辅因子裸受体、`fep_md` 配体库），正式 APO/OPEN RBFE 已提交运行。执行细节与全部资产/任务 ID 见[隐式口袋闭环验证执行参考](隐式口袋闭环验证执行参考.md)。
...
| **第二阶段 (Week 2)** | 用 `wa-dd-gromacs` 跑通 L0 基准 FEP + L2 物理验证。 | L0 进行中（server6 运行）；L2 待 L1 产出 |
