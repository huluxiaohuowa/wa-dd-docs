...
- [x] 正式 L0 OPEN RBFE：任务 `L0_RBFE_OPEN_6GJ8_bare_KRAS_lib_fep`（`484407ba-ccfa-4017-94b0-f75ea8741cec`）提交后正常推进，2026-08-24 14:18 CST 已进入最终分析单元（edge_001 MBAR）；随后 server6 网络故障（主机端口可连但 sshd/web 立即断连），最终状态待恢复后确认。若 worker 因主机重启中断，用任务详情页的重试或 `POST /api/v1/jobs/{id}/retry` 重跑。
- [ ] OPEN 完成后：下载两组 `rbfe_results.json` / `rbfe_results.csv` 与完整输出包。
- [ ] L0 判定：ΔΔΔG = ΔΔG(OPEN) − ΔΔG(APO)，以 Adagrasib 边为准；若 OPEN 边显著更负（差 ≥ 1.0 kcal/mol），支持“大分子量配体偏好打开构象”，L0 通过。Sotorasib 边（-99）重叠度太差，不参与判定。
