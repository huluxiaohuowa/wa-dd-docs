> [English documentation](admin.EN.md)

# 管理

## 作用

管理页面从右上角“当前用户”菜单中的“管理”进入，不再作为顶部模块导航按钮展示。该用户菜单同时承载界面语言、修改密码、本用户文件、本用户任务和退出入口。管理页面内容仍用于管理用户、项目文件和全局任务，并查看部署机器资源；普通用户只能查看和操作自己的项目、资产与任务。
...
![右上角当前用户菜单中的管理入口，以及管理与系统资源页面](images/admin-system-resources.jpg)

## 使用方式

1. 以管理员身份登录后，点击右上角用户菜单，再打开“管理”。
